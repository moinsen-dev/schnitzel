...
class DartModelGenerator:
    ...
    def _needs_json_key(self, field_name: str, dart_field_name: str) -> bool:
        """Check if a field needs @JsonKey annotation.

        A field needs @JsonKey if the Dart field name differs from the JSON key name.

        Args:
            field_name: Original field name (JSON key, typically snake_case)
            dart_field_name: Dart field name (lowerCamelCase)

        Returns:
            True if @JsonKey annotation is needed
        """
        return field_name != dart_field_name
    ...
    def generate(self, schema: SchnitzelSchema) -> str:
        ...
        # Add base imports for Freezed (includes JSON serialization annotations)
    ...
    def generate_to_file(self, schema: SchnitzelSchema, output_dir: str | Path, schema_source: str = "schema.schnitzel.yaml", dry_run: bool = False) -> tuple[Path, int]:
        ...
        # Build header comment with ignore directives for Freezed false positives
...
"""
    ...
    def _generate_model(self, model: Model) -> str:
        ...
        lines.extend(["@freezed", f"abstract class {model.name} with _${model.name} {{"])
    ...
    def _generate_relationship_field(self, relation_name: str, relation_def: Relation) -> str:
        ...
        Args:
            relation_name: Name of the relationship field (from schema, may be snake_case)
        ...
        """
        # Convert relation name to Dart lowerCamelCase
        dart_field_name = self._to_camel_case(relation_name)

        # Handle belongsTo relationship
        if relation_def.type == "belongsTo":
            json_key = ""
            if self._needs_json_key(relation_name, dart_field_name):
                json_key = f"@JsonKey(name: '{relation_name}') "
            return f"{json_key}{relation_def.model}? {dart_field_name},"
        ...
        if relation_def.type == "hasMany":
            ...
            if self._needs_json_key(relation_name, dart_field_name):
                json_key = f"@JsonKey(name: '{relation_name}') "
            return f"{json_key}List<{relation_def.model}>? {dart_field_name},"
    ...
    def _generate_field(self, field_name: str, field_def: FieldDefinition) -> str:
        ...
        Args:
            field_name: Name of the field (from schema, may be snake_case)
        ...
        # Convert field name to Dart lowerCamelCase
        ...
        # Check if we need @JsonKey annotation (when JSON key differs from Dart field name)
        if self._needs_json_key(field_name, dart_field_name):
            annotations.append(f"@JsonKey(name: '{field_name}')")
        ...
        # Build the field declaration using Dart lowerCamelCase field name
        field_declaration = f"{annotation_str}{required_keyword}{dart_type} {dart_field_name},"
